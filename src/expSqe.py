...
wherever data exists."""
    ...
    print "In updateMask..."
    print "len(q), len(e) : ", len(self.q), len(self.e)

    #initialize mask to be all 1 (use all data) by default
    mask = numpy.zeros(self.shape,'i')+1

    #loop over values in q,e space, set to 0 where value of s(q,e) is zero
    #in order to mask data.
    for q in range(len(self.q)-1):
      for e in range(len(self.e)-1):
        if self.sqe[q,e] == 0:
          mask[q,e] = 0
    self.mask = mask
    return 
...
"""
    ...
    # here we are battling values that are large negative infinities
    # after background subtraction due to roundoff errors. 
    # currently hacked to set values that are very large negatives to be 
    # zeroed so they don't contribute
    # in testing, these values were all confined to energy gain values
    # in principle, these values should be added to a mask, not zeroed as done
    # here.
    self.sqe[numpy.where(self.sqe < -10000)]=0
...
"""
    ...
    print "In instantiation, size of q: ", len(self.q)
    print "In instantiation, size of e: ", len(self.e)
    print "In instantiation, shape of sqe: ", numpy.shape(self.sqe)
    return
