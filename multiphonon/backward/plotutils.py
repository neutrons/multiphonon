...
def plot_dos_iteration(curdir, total_rounds=None):
    ...
    for round_no in range(total_rounds):
        ...
        dos = hh.load(fn)
        plt.errorbar(dos.E, dos.I, dos.E2**0.5, label=str(round_no))
        continue
    plt.legend()
    return
...
def plot_residual(curdir):
    ...
    E = exp_pos_se.E
    plt.errorbar(E, exp_pos_se.I, exp_pos_se.E2**0.5, label="exp S(E)")
    plt.errorbar(E, residual_pos_se.I, residual_pos_se.E2**0.5, label="residual")
...
def plot_intermediate_result_sqe(curdir):
    ...
    for index, (title, fn) in enumerate(plots):
        ...
        if title == "exp-residual":
            zmin, zmax = np.array([-1.0, 1]) / 2.0 * zmax
...
def plot_intermediate_result_se(curdir):
    ...
    for index, (title, fn) in enumerate(plots):
        ...
        if title.startswith("sim"):
            ...
            plt.errorbar(E, se.I, se.E2**0.5, ls="none", elinewidth=2, label=title)
    ...
    plt.ylim(-max_inel_I / 10.0, max_inel_I * 1.1)
    plt.legend()
    return
