...
def compute_scalefactor_using_area_criteria(
    original_dos_hist, new_dos_hist, Emin, Emax
):
...
def compute_scalefactor_using_continuous_criteria(
    original_dos_hist, new_dos_hist, Emin, Emax, Npoints=3
):
    ...
    if bracket[1] > new_dos_hist.E[-1]:
        raise RuntimeError(
            "Stitching point %s too large and close to the max energy transfer for new DOS %s"
...
class DOSStitcher(DOSStitcherBase):
    ...
    def match(self, original_dos_hist, new_dos_hist, Emin, Emax):
        ...
        scale1 = compute_scalefactor_using_continuous_criteria(
            original_dos_hist, new_dos_hist, Emin, Emax, Npoints=3
        )
        scale2 = compute_scalefactor_using_area_criteria(
            original_dos_hist, new_dos_hist, Emin, Emax
        )
