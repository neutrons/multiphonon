...
from sqe2dos_not_complete_dos import sqe2dos
from multiphonon_not_complete_dos import getSQE
from multiphonon_not_complete_dos import *
from sqePlot import plotSQE, plotComp

def getCorrectedScatter(sqe,C_ms,N,Tol,g0,interactive,vd=False):
#---docstring------------------------------------------------------------------
  """
...
g0 = thermal factor from full dos
...
#-----------------------------------------------------------------
  for x in C_ms:
    ...
    if interactive:
      ...
      sys.stdout.flush()
    dos = sqe2dos(sqe0,g0)
    dos0 = Dos(dos.e,dos.g)
    another = True
    while another == True:
      SQE  = getSQE(sqe0,dos,N,x,g0)
      sqe.sqe = sqe0.sqe - nar(SQE[1:])
      dos = sqe2dos(sqe,g0)
      if nar( (dos.gz-dos0.gz)**2.0 ) > Tol:
        print nar( (dos.gz-dos0.gz)**2 )
        dos0 = Dos(dos.e,dos.g)
        another = False
      else:
        another = False
...
#---end-docstring--------------------------------------------------------------
  ...
  start = 0
  while numpy.isnan(lsqSc[start]) or numpy.isinf(lsqSc[start]) \
